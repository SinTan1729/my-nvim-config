...
if [ "$1" = "push" ]; then
    ...
    rsync -ahP --exclude={'*.md','*.sh','LICENSE','.*'} "$DIR/" "$HOME/.config/nvim/" --delete-excluded
    for server in server vps pi3b pizero; do
        if ping -qc2 -W5 $server-ts >/dev/null; then
            echo "Sending to $server..."
            rsync -ahP --exclude={'*.md','*.sh','LICENSE','.*','snips/','spell/'} "$DIR/" $server-ts-rsync:"~/.config/nvim/" --delete-excluded
        else
